...
/**
 * Manually check network connectivity by making a lightweight request
 * This helps in cases where the browser doesn't accurately report online status
 * @returns {Promise<boolean>} True if the request succeeds, false otherwise
 */
export const checkConnectivity = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/ping?_=${Date.now()}`, {
      ...
      // Short timeout to avoid long waits in case of slow connections
      // Note: AbortController is used in modern browsers
    ...
    // If the check indicates a different state than what's currently stored,
    // update the state and trigger appropriate callbacks
    ...
    // If we can't make the request, assume we're offline
    if (isNetworkOnline) {
      isNetworkOnline = false;
      handleOffline();
    }
 ...
 * @returns {Function} Function to stop the periodic checks
 */
export const startPeriodicConnectivityChecks = (interval = 30000) => {
  if (typeof window === 'undefined') {
    return () => {}; // SSR - nothing to do
  }
  
  // Set up a check that runs at the specified interval
  const timerId = setInterval(checkConnectivity, interval);
  
  // Run an initial check
  checkConnectivity();
  
  // Return a function to cancel the periodic checks
