...
import ToolEffectivenessView from "../../views/reports/ToolEffectivenessView";
 ...
 */
const ToolEffectivenessViewModel = ({ reports, filters = {} }) => {
  ...
  useEffect(() => {
    ...
    filteredEntries.forEach((entry) => {
      ...
      toolGroups[toolName].roles[role] =
        (toolGroups[toolName].roles[role] || 0) + 1;

      // Track task distribution
      const task = entry.sdlcTask || "Not Specified";
      toolGroups[toolName].tasks[task] =
        (toolGroups[toolName].tasks[task] || 0) + 1;

      // Track SDLC step distribution
      const sdlcStep = entry.sdlcStep || "Not Specified";
      toolGroups[toolName].sdlcSteps[sdlcStep] =
        (toolGroups[toolName].sdlcSteps[sdlcStep] || 0) + 1;
    });

    // Calculate averages and prepare final data
    const toolsComparison = Object.values(toolGroups)
      .map((tool) => {
        // Calculate average time saved
        const averageSaved =
          tool.totalTasks > 0 ? tool.totalSaved / tool.totalTasks : 0;
        ...
        const tasksArray = Object.entries(tool.tasks)
          .map(([name, count]) => ({
            name,
            count,
            percentage: (count / tool.totalTasks) * 100,
        ...
        const sdlcStepsArray = Object.entries(tool.sdlcSteps)
          .map(([name, count]) => ({
            name,
            count,
            percentage: (count / tool.totalTasks) * 100,
    ...
    setToolsData({
      toolsComparison,
      roleDistribution: Object.values(roleDistribution).sort((a, b) => {
        // Calculate total saved time for each role
        const aSaved = a.toolBreakdown.reduce(
          (sum, item) => sum + item.saved,
          0,
        );
        const bSaved = b.toolBreakdown.reduce(
          (sum, item) => sum + item.saved,
          0,
        );
        return bSaved - aSaved;
      }),
      taskDistribution: Object.values(taskDistribution).sort((a, b) => {
  ...
  return <ToolEffectivenessView toolsData={toolsData} filters={filters} />;
};

export default ToolEffectivenessViewModel;
