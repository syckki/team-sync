...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to retrieve reference data" });
    }
  }
  ...
  if (req.method === "POST") {
    try {
      ...
      if (!newData || Object.keys(newData).length === 0) {
        return res
          .status(400)
          .json({ error: "Invalid reference data provided" });
      }
      ...
      if (!success) {
        return res
          .status(500)
          .json({ error: "Failed to update reference data" });
      }

      return res
        .status(200)
        .json({ message: "Reference data updated successfully" });
    } catch (error) {
