...
export const addMessageToThread = async (threadId, encryptedData, metadata = {}, threadTitle = null) => {
  ...
  // Check if this is an update to an existing draft report
  let messageIndex;
  let isThreadCreator = false;
  let existingDraft = null;
  
  // If this is a report and has an authorId and is in draft status, check for existing drafts
  if (metadata.isReport && metadata.authorId && metadata.status === 'draft') {
