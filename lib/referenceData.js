...
import { mergeData } from "./mergeUtils";
 ...
 */
export const updateReferenceData = async (data) => {
  try {
    const allData = await getAllReferenceData();
    if (!allData) return false;

    // Combine existing data with new data
    const { altered, ...newData } = data;
    const combinedData = mergeData(allData, newData, altered);
