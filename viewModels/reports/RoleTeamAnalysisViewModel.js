...
import RoleTeamAnalysisView from "../../views/reports/RoleTeamAnalysisView";
 ...
 */
const RoleTeamAnalysisViewModel = ({ reports, filters = {} }) => {
    ...
    return reports.filter((report) => {
      ...
      if (filters.aiTool) {
        const hasMatchingEntry = report.entries.some(
          (entry) => entry.aiTool === filters.aiTool,
        );
  ...
  useEffect(() => {
    ...
    const roleAnalysis = Object.values(roleData)
      .map((role) => {
        // Convert aiTools object to sorted array
        const aiToolsArray = Object.values(role.aiTools).sort(
          (a, b) => b.hoursSaved - a.hoursSaved,
        );

        // Get the most used tools (top 3)
        const mostUsedTools = aiToolsArray.slice(0, 3).map((tool) => tool.tool);
        ...
        return {
          ...
          averageHoursSaved:
            role.totalTasks > 0 ? role.totalHoursSaved / role.totalTasks : 0,
    ...
    const teamMemberAnalysis = Object.values(teamMemberData)
      .map((member) => {
        // Convert aiTools object to sorted array
        const aiToolsArray = Object.values(member.aiTools).sort(
          (a, b) => b.hoursSaved - a.hoursSaved,
        );
    ...
    const teamAnalysis = Object.values(teamData)
      .map((team) => {
        // Convert aiTools object to sorted array
        const aiToolsArray = Object.values(team.aiTools).sort(
          (a, b) => b.hoursSaved - a.hoursSaved,
        );
        ...
        return {
          ...
          averageHoursSaved:
            team.totalTasks > 0 ? team.totalHoursSaved / team.totalTasks : 0,
    ...
    const adoptionRatesByRole = roleAnalysis
      .map((role) => {
        ...
        const adoptionRate = Math.min(
          100,
          (role.teamMemberCount / allTeamMembers.length) * 100,
        );
    ...
    const adoptionRatesByTeam = teamAnalysis
      .map((team) => {
        // Estimate the total size of the team
        // This is just an approximation - in a real app, you'd have actual team sizes
        const totalTeamSize = Math.max(
          team.teamMemberCount,
          Math.ceil(allTeamMembers.length / allTeamNames.size),
        );
        const adoptionRate = Math.min(
          100,
          (team.teamMemberCount / totalTeamSize) * 100,
        );
    ...
    Object.entries(roleToolUsage).forEach(([role, tools]) => {
      aiToolUsageByRole[role] = Object.entries(tools)
        .map(([tool, stats]) => ({
          tool,
    ...
    Object.entries(teamToolUsage).forEach(([team, tools]) => {
      aiToolUsageByTeam[team] = Object.entries(tools)
        .map(([tool, stats]) => ({
          tool,
          hoursSaved: stats.hoursSaved,
          usageCount: stats.usageCount,
  ...
  return <RoleTeamAnalysisView analysisData={analysisData} filters={filters} />;
};

export default RoleTeamAnalysisViewModel;
