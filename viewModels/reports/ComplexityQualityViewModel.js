...
import ComplexityQualityView from "../../views/reports/ComplexityQualityView";
 ...
 */
const ComplexityQualityViewModel = ({ reports, filters = {} }) => {
  ...
  useEffect(() => {
    ...
    const processedEntries = filteredEntries.map((entry) => {
      ...
      if (entry.complexity) {
        ...
        if (typeof complexityValue === "string") {
          if (
            complexityValue.includes("low") ||
            complexityValue.includes("simple") ||
            complexityValue === "1"
          ) {
            complexity = "low";
          } else if (
            complexityValue.includes("high") ||
            complexityValue.includes("complex") ||
            complexityValue === "3"
          ) {
            complexity = "high";
          } else {
          ...
          if (
            complexityValue <= 1.5 ||
            (complexityValue <= 2 && complexityValue <= 5)
          ) {
            complexity = "low";
          } else if (
            complexityValue >= 2.5 ||
            (complexityValue >= 4 && complexityValue <= 5)
          ) {
            complexity = "high";
          } else {
            complexity = "medium";
        ...
        if (
          taskDetails.includes("simple") ||
          taskDetails.includes("easy") ||
          taskDetails.includes("straightforward") ||
          taskDetails.includes("basic")
        ) {
          complexity = "low";
        } else if (
          taskDetails.includes("complex") ||
          taskDetails.includes("difficult") ||
          taskDetails.includes("challenging") ||
          taskDetails.includes("advanced")
        ) {
          complexity = "high";
        }
      ...
      if (entry.qualityImpact) {
        const qualityValue =
          typeof entry.qualityImpact === "string"
            ? entry.qualityImpact.toLowerCase()
            : entry.qualityImpact;

        if (typeof qualityValue === "string") {
          if (
            qualityValue.includes("improve") ||
            qualityValue.includes("better") ||
            qualityValue.includes("enhanced") ||
            qualityValue === "good"
          ) {
            qualityImpact = "improved";
          } else if (
            qualityValue.includes("rework") ||
            qualityValue.includes("poor") ||
            qualityValue.includes("issue") ||
            qualityValue.includes("fix") ||
            qualityValue === "bad"
          ) {
        ...
        if (
          taskDetails.includes("better quality") ||
          taskDetails.includes("improved quality") ||
          taskDetails.includes("enhanced quality") ||
          taskDetails.includes("higher quality") ||
          aiProductivity.includes("better quality") ||
          aiProductivity.includes("improved quality")
        ) {
          qualityImpact = "improved";
        } else if (
          taskDetails.includes("rework") ||
          taskDetails.includes("fix") ||
          taskDetails.includes("issue") ||
          taskDetails.includes("problem") ||
          aiProductivity.includes("needs review") ||
          aiProductivity.includes("requires fixes")
        ) {
          qualityImpact = "needsRework";
        }
      }
    ...
    processedEntries.forEach((entry) => {
      const {
        ...
        sdlcStep = "Not Specified",
        teamRole = "Not Specified",
      } = entry;
    ...
    Object.keys(byComplexity).forEach((key) => {
      byComplexity[key].averageHoursSaved =
        byComplexity[key].count > 0
          ? byComplexity[key].totalHoursSaved / byComplexity[key].count
          : 0;
    });

    Object.keys(byQuality).forEach((key) => {
      byQuality[key].averageHoursSaved =
        byQuality[key].count > 0
    ...
    const allHoursSaved = Object.values(timeSavedCategories)
      .map((item) => item.hoursSaved)
      .sort((a, b) => a - b);
    const lowThreshold =
      allHoursSaved.length > 0
    ...
    Object.values(timeSavedCategories).forEach(
      ({ hoursSaved, qualityImpact }) => {
        let category;
    ...
    Object.keys(toolsByComplexity).forEach((complexity) => {
      toolsByComplexityArray[complexity] = Object.values(
        toolsByComplexity[complexity],
      ).sort((a, b) => b.totalHoursSaved - a.totalHoursSaved);
    });

    const toolsByQualityArray = {};
    Object.keys(toolsByQuality).forEach((quality) => {
      toolsByQualityArray[quality] = Object.values(
        toolsByQuality[quality],
      ).sort((a, b) => b.totalHoursSaved - a.totalHoursSaved);
    });

    // Convert complexity breakdowns to sorted arrays
    const complexityBreakdownsArray = {
      bySDLC: Object.values(complexityBreakdowns.bySDLC).sort(
        (a, b) => b.totalHoursSaved - a.totalHoursSaved,
      ),
      byRole: Object.values(complexityBreakdowns.byRole).sort(
        (a, b) => b.totalHoursSaved - a.totalHoursSaved,
      ),
      byTool: Object.values(complexityBreakdowns.byTool).sort(
        (a, b) => b.totalHoursSaved - a.totalHoursSaved,
      ),
    };
  ...
  return (
    <ComplexityQualityView analysisData={analysisData} filters={filters} />
  );
};

export default ComplexityQualityViewModel;
