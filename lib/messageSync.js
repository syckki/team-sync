...
export const initMessageSyncManager = () => {
  ...
  // Setup service worker event listeners if available
  if ('serviceWorker' in navigator) {
    try {
      ...
      navigator.serviceWorker.ready
        .then(registration => {
          // Check if sync API is available (it won't be in many browsers)
          if (registration.sync && typeof registration.sync.register === 'function' && 'SyncManager' in window) {
            try {
              ...
              console.log('Error when registering for background sync, falling back to polling', err);
            }
          } else {
            console.log('Background Sync API not supported in this browser, using polling for offline sync');
          }
        })
        .catch(err => {
          console.log('Service worker registration not ready, falling back to polling', err);
        });
    } catch (err) {
      console.log('Error setting up service worker events, falling back to polling', err);
    }
  } else {
    console.log('Service Workers not supported in this browser, using polling for offline sync');
  }
