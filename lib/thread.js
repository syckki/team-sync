...
/**
 * Create a new thread or add a message to an existing thread, or update an existing message
 ...
 * @param {number} updateMessageIndex - Optional index of the message to update (if null, creates a new message)
 * @returns {object} Thread info with ID and message index
 */
export const addMessageToThread = async (threadId, encryptedData, metadata = {}, threadTitle = null, updateMessageIndex = null) => {
  ...
  if (updateMessageIndex !== null && !isNaN(updateMessageIndex)) {
    messageIndex = parseInt(updateMessageIndex);
    
    // Verify the message exists
    const existingMessagePath = path.join(threadDir, `${messageIndex}.bin`);
    if (!fs.existsSync(existingMessagePath)) {
      throw new Error(`Message with index ${messageIndex} does not exist in thread ${threadId}`);
    }
    
    // Check if the author ID matches (if provided) for security
    if (metadata.authorId) {
      const existingMetadataPath = path.join(threadDir, `${messageIndex}.meta.json`);
      if (fs.existsSync(existingMetadataPath)) {
        try {
          const existingMetadata = JSON.parse(fs.readFileSync(existingMetadataPath, 'utf8'));
          if (existingMetadata.authorId !== metadata.authorId) {
            throw new Error('You do not have permission to update this message');
          }
          
          // Preserve the thread creator status
          isThreadCreator = existingMetadata.isThreadCreator || false;
        } catch (error) {
          console.error(`Error reading metadata for message ${messageIndex}:`, error);
          throw new Error('Failed to verify message ownership');
        }
      }
    }
    
    console.log(`Updating existing message ${messageIndex} in thread ${threadId}`);
  } else {
    // Get current message count to determine index for a new message
  ...
  if (isThreadCreator && updateMessageIndex === null) { // Only do this for new messages
  ...
  const finalMetadata = {
    ...
    isThreadCreator,
    ...metadata,
    lastUpdated: updateMessageIndex !== null ? new Date().toISOString() : undefined
  };
  ...
  const totalMessages = fs.readdirSync(threadDir).filter(f => f.endsWith('.bin')).length;
  ...
  return {
    ...
    isThreadCreator,
    isUpdate: updateMessageIndex !== null
  };
};
